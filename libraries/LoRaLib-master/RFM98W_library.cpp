...
uint8_t RFMLib::checkInterrupt()
{
   	//uint8_t TxDone =  0x40; //if mapping reg 0x40
	//if((intReg & TxDone) = TxDone) { rfm_done = true; return TxDone;}
	//uint8_t RxDone  = 0x40|0x80;
	//if((RxDone & intReg) == 0) { }

	uint8_t intReg = digitalRead(dio0);//pin
	if(intReg) { rfm_done = true; }
	return intReg;
} 

bool RFMLib::pullInterruptRx()
{
	uint8_t intReg = rRFM(0x40);
	uint8_t RxDone =  0x40|0x80; 
	if((intReg & RxDone) == 0) { return true; }
	else return false;
}

bool RFMLib::pullInterruptTx()
{
	uint8_t intReg = rRFM(0x40);
	uint8_t TxDone =  0x40; //if mapping reg 0x40
	if((intReg & TxDone) == TxDone) { return true; }
	else return false;
	
}

void RFMLib::configure(byte config[6]){//{0x44,0x84,0x88,0xAC,0xCD, 0x08};
    pinMode(nss,OUTPUT);
    ...
    pinMode(dio5,INPUT);
    radioMode(0);
    wRFM(0x1D,config[0]);//31.25 kHz,Explicit Header mode , Error coding rate  = 4/8
    wRFM(0x1E,config[1]);//modem config registers: Header indicates CRC on, SF rate = 256 chips / symbol 
    wRFM(0x09,config[2]);//use PA_BOOST - even at the same power setting: Rise/Fall time of ramp up/down in FSK = 50 us, PA_BOOST pin. Output power is limited to +20 dBm
